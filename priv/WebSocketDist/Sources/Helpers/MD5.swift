...
public extension Sequence where Element == UInt8 {
    ...
    ///     }
    var md5: MD5Digest {
        return MD5Digest(from: Array(self))
    }
}
...
public struct MD5Digest : Hashable, RawRepresentable, LosslessStringConvertible, Codable {
    ...
    public var bytes: [UInt8] {
        var v = self
        return withUnsafeBytes(of: &v) { (ptr: UnsafeRawBufferPointer) -> [UInt8] in
            return Array(ptr)
        }
    }
}
...
fileprivate struct MD5State {
    ...
    init(_ message: [UInt8]) {
        ...
        var chunk = [UInt8](repeating: 0, count: MD5State.chunkSize)
        chunk.replaceSubrange(0..<remainingByteCount, with: message[(message.count - remainingByteCount)...])
        ...
        if remainingByteCount >= 56 {
            ...
            chunk = [UInt8](repeating: 0, count: MD5State.chunkSize)
        }
    ...
    private mutating func feedFullChunks(in message: [UInt8]) -> Int {
        let chunkCount = message.count / MD5State.chunkSize
        message.withUnsafeBytes { (pointer: UnsafeRawBufferPointer) -> Void in
            var cursor = pointer.bindMemory(to: UInt32.self).baseAddress!
